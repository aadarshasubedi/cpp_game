...
bool isLegal(const vector<char>&, int move);
...
char humanPiece() {
  char go_first = ask("Do you require the first move?(y/n)");
  if(go_first == 'y') {
    cout << "\nThen take the first move. You will need it.\n";
    return X;
  } else {
    cout << "\nYour bavery will be your undoing... I will do first.\n";
    return O;
  }
...
char winner(const vector<char>& board) {
  const int ROW_TOTAL = 8;
  const int WINNING_ROW[8][3] = {
    {0, 1, 2},
    {3, 4, 5},
    {6, 7, 8},
    {0, 4, 8},
  ...
  for(int row = 0; row < ROW_TOTAL; row++) {
    if( (board[WINNING_ROW[row][0]] != EMPTY) &&
        ...
        (board[WINNING_ROW[row][1]] == board[WINNING_ROW[row][2]]) ) {
